    ...
    private fun seek(args: Any?) {
        if (args is Map<*, *>) {
            ...
            if (relative) {
                ...
                interval = interval?.plus(sessionManager?.currentCastSession?.remoteMediaClient?.mediaStatus?.streamPosition
                        ?: 0)
            ...
            val request = sessionManager?.currentCastSession?.remoteMediaClient?.seek(interval?.toLong()
                    ?: 0)
            sessionManager?.currentCastSession?.remoteMediaClient?.mediaStatus?.mediaInfo?.streamDuration
